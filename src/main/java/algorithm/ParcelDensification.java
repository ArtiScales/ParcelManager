...
public class ParcelDensification {
   ...
   * @throws Exception
   */
  public static SimpleFeatureCollection parcelDensification(SimpleFeatureCollection parcelCollection, SimpleFeatureCollection ilotCollection, File tmpFolder, File buildingFile,
      double maximalAreaSplitParcel, double minimalAreaSplitParcel, double maximalWidthSplitParcel, double lenDriveway, boolean isArt3AllowsIsolatedParcel) throws Exception {
    ...
    try {
      while (iterator.hasNext()) {
        ...
        if (iFeat.getAttribute("SPLIT").equals(1) && ((Geometry) iFeat.getDefaultGeometry()).getArea() > maximalAreaSplitParcel) {
          ...
          SimpleFeatureCollection tmp = ParcelSplitFlag.generateFlagSplitedParcels(iFeat, lines, tmpFolder, buildingFile, maximalAreaSplitParcel, maximalWidthSplitParcel,
